...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if (Status != XST_SUCCESS)
		{
			xil_printf ("Invalid Slave Select: %d \n", Status);
		}
	BYTE WriteData[2];
	WriteData[0] = reg + 2;
	WriteData[1] = val;
	int return_code =  XSpi_Transfer(&SpiInstance,WriteData,NULL,2);
	if(return_code != 0)
	{
		xil_printf ("Error. Invalid Return Code: %d \n", return_code);
	}
	Status =  XSpi_SetSlaveSelect(&SpiInstance,0x00000000);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code =  XSpi_Transfer(&SpiInstance,WriteData,NULL,nbytes+1);
	if(return_code != 0)
		{
			xil_printf ("Error. Invalid Return Code: %d \n", return_code);
		}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE val[2];
	val[0] = reg;
	int return_code = XSpi_Transfer(&SpiInstance,val,val,2);
	if(return_code != 0)
		{
			xil_printf ("Error. Invalid Return Code: %d \n", return_code);
		}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE read_data[nbytes + 1];
	read_data[0] = reg;
	int return_code = XSpi_Transfer(&SpiInstance,read_data,read_data,nbytes+1);
	if(return_code != 0)
				{
					xil_printf ("Error. Invalid Return Code: %d \n", return_code);
				}
