...
module sdcard_init (
    input  logic GO,
	input  logic clk50,
	input  logic reset, sample_mode, reverse,       //starts as soon reset is deasserted
	...
	output logic [31:0] maxx, monitor,
...
assign cur_state = state_r; //2800ss + 350 is sound select offset, start_incr * 7 is position offset, 
 ...
 //converts sector offset to address offset. why is it 256 and not 512? idk
always_comb begin
    if (sampler) begin
        sector_offset = (ss * 2800) + 350;
        MAX_RAM_ADDRESS = 1720 + sector_offset;
        start_addr = sector_offset * 256;
    end else if (rev) begin
        sector_offset = (ss * 2800) + 350 + (start_incr * 673 / 100);
        MAX_RAM_ADDRESS = sector_offset;
        start_addr = ((length_incr * 250 / 100 + 15) + sector_offset) * 256;
   end else begin // normal condition
        sector_offset = (ss * 2800) + 350 + (start_incr * 673 / 100);
        MAX_RAM_ADDRESS = (length_incr * 250 / 100 + 15) + sector_offset;
        start_addr = sector_offset * 256;
   end
end

assign maxx = MAX_RAM_ADDRESS;
assign monitor = start_addr;
...
begin
	...
	unique case (state_r)
		...
		READBLOCK: begin //send enable to start block read
			if ((((sd_block_addr >= MAX_RAM_ADDRESS) || (sd_block_addr >= sector_offset + 1720)) && (rev == 1'b0)) || ((sd_block_addr <= MAX_RAM_ADDRESS) && (rev == 1'b1))) //done with the whole range
				state_x = DONE;
